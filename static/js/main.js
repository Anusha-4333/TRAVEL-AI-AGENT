document.addEventListener('DOMContentLoaded', () => {
  ...
  if (chatForm) {
    chatForm.dataset.bound = '1'; // Prevent duplicate fallback binding
  ...
  if (planForm) {
    planForm.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        const parsedPlan = json.plan || null; // structured object when available
        ...
        // Prefer structured plan when available
        if (parsedPlan && typeof parsedPlan === 'object') {
          if (parsedPlan.overview) {
            html += `<p>${escapeHtml(parsedPlan.overview)}</p>`;
          }

          const itinerary = Array.isArray(parsedPlan.itinerary) ? parsedPlan.itinerary : (Array.isArray(parsedPlan) ? parsedPlan : null);
          if (itinerary) {
  ...
  if (featureLearnMoreBtn) {
    featureLearnMoreBtn.addEventListener('click', () => {
      // Learn more can scroll to services or external link
      closeFeaturePopup();
  ...
  // Log on page load to confirm scripts are working
